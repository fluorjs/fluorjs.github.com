...
export default function ScriptingAPI() {
  return (
    <Section id="api" title="Scripting API">
      ...
      >
        ...
        <Info>
          <Info.P>
            You cannot directly store a function in a variable since it would
            recognize it as an <CFunction>updater</CFunction>. To store a
            function, pass an <CFunction>updater</CFunction> that returns a
            function itself.
          </Info.P>
          <Source className="mt-2">{`set("add", () => (a, b) => a + b)`}</Source>
        </Info>
