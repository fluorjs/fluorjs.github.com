...
export default function APIMember({ children, member, type, ...props }) {
  const Signature = SIGNATURE_COMPONENTS[type]
  const Label = LABEL_COMPONENTS[type]
  return (
    <section id={`api-${member}`} className="mt-4 mb-16">
...
APIMember.Examples = function Examples({ children }) {
  return (
    <aside className="my-8">
      <h4 className="uppercase text-gray-700 font-bold">Examples</h4>
      {children}
    </aside>
  )
}
...
function CallableSignatureFor(MemberTypeComponent) {
  return function CallableSignature({ member, signatures }) {
    return (
      <ul className="text-sm p-4 border border-solid border-gray-400 bg-gray-900 text-gray-300 rounded-lg">
